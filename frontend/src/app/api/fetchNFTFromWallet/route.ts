...
const LINEA_RPC_URL = {
  mainnet: `https://linea-mainnet.infura.io/v3/${INFURA_API_KEY}`,
  testnet: `https://linea-goerli.infura.io/v3/${INFURA_API_KEY}`,
};
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const [collectionsResponse, nfts] = await Promise.all([
      collectionsPromise,
      nftsPromise,
    ]);
